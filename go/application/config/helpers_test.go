package config

import (
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestGetString(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name    string
		config  Config
		key     string
		wantVal string
		wantOK  bool
	}{
		{
			name:    "string value found",
			config:  Config{"hostname": "example.com"},
			key:     "hostname",
			wantVal: "example.com",
			wantOK:  true,
		},
		{
			name:    "key not found",
			config:  Config{"other": "value"},
			key:     "hostname",
			wantVal: "",
			wantOK:  false,
		},
		{
			name:    "wrong type",
			config:  Config{"hostname": 123},
			key:     "hostname",
			wantVal: "",
			wantOK:  false,
		},
		{
			name:    "nil config",
			config:  nil,
			key:     "hostname",
			wantVal: "",
			wantOK:  false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			val, ok := GetString(tt.config, tt.key)
			assert.Equal(t, tt.wantVal, val)
			assert.Equal(t, tt.wantOK, ok)
		})
	}
}

func TestGetInt(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name    string
		config  Config
		key     string
		wantVal int
		wantOK  bool
	}{
		{
			name:    "int value",
			config:  Config{"port": 443},
			key:     "port",
			wantVal: 443,
			wantOK:  true,
		},
		{
			name:    "float64 value (JSON default)",
			config:  Config{"port": float64(443)},
			key:     "port",
			wantVal: 443,
			wantOK:  true,
		},
		{
			name:    "int64 value",
			config:  Config{"port": int64(443)},
			key:     "port",
			wantVal: 443,
			wantOK:  true,
		},
		{
			name:    "string value - wrong type",
			config:  Config{"port": "443"},
			key:     "port",
			wantVal: 0,
			wantOK:  false,
		},
		{
			name:    "key not found",
			config:  Config{},
			key:     "port",
			wantVal: 0,
			wantOK:  false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			val, ok := GetInt(tt.config, tt.key)
			assert.Equal(t, tt.wantVal, val)
			assert.Equal(t, tt.wantOK, ok)
		})
	}
}

func TestGetBool(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name    string
		config  Config
		key     string
		wantVal bool
		wantOK  bool
	}{
		{
			name:    "true value",
			config:  Config{"enabled": true},
			key:     "enabled",
			wantVal: true,
			wantOK:  true,
		},
		{
			name:    "false value",
			config:  Config{"enabled": false},
			key:     "enabled",
			wantVal: false,
			wantOK:  true,
		},
		{
			name:    "string value - wrong type",
			config:  Config{"enabled": "true"},
			key:     "enabled",
			wantVal: false,
			wantOK:  false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			val, ok := GetBool(tt.config, tt.key)
			assert.Equal(t, tt.wantVal, val)
			assert.Equal(t, tt.wantOK, ok)
		})
	}
}

func TestGetStringSlice(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name    string
		config  Config
		key     string
		wantVal []string
		wantOK  bool
	}{
		{
			name:    "valid string slice",
			config:  Config{"tags": []interface{}{"a", "b", "c"}},
			key:     "tags",
			wantVal: []string{"a", "b", "c"},
			wantOK:  true,
		},
		{
			name:    "empty slice",
			config:  Config{"tags": []interface{}{}},
			key:     "tags",
			wantVal: []string{},
			wantOK:  true,
		},
		{
			name:    "mixed types - fails",
			config:  Config{"tags": []interface{}{"a", 123}},
			key:     "tags",
			wantVal: nil,
			wantOK:  false,
		},
		{
			name:    "key not found",
			config:  Config{},
			key:     "tags",
			wantVal: nil,
			wantOK:  false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			val, ok := GetStringSlice(tt.config, tt.key)
			assert.Equal(t, tt.wantVal, val)
			assert.Equal(t, tt.wantOK, ok)
		})
	}
}

func TestMustGetString(t *testing.T) {
	t.Parallel()

	t.Run("success", func(t *testing.T) {
		t.Parallel()
		config := Config{"hostname": "example.com"}
		val, err := MustGetString(config, "hostname")
		require.NoError(t, err)
		assert.Equal(t, "example.com", val)
	})

	t.Run("missing key", func(t *testing.T) {
		t.Parallel()
		config := Config{}
		_, err := MustGetString(config, "hostname")
		require.Error(t, err)
		assert.Contains(t, err.Error(), "hostname")
	})

	t.Run("wrong type", func(t *testing.T) {
		t.Parallel()
		config := Config{"hostname": 123}
		_, err := MustGetString(config, "hostname")
		require.Error(t, err)
	})
}

func TestGetStringDefault(t *testing.T) {
	t.Parallel()

	t.Run("value exists", func(t *testing.T) {
		t.Parallel()
		config := Config{"hostname": "custom.com"}
		val := GetStringDefault(config, "hostname", "default.com")
		assert.Equal(t, "custom.com", val)
	})

	t.Run("uses default", func(t *testing.T) {
		t.Parallel()
		config := Config{}
		val := GetStringDefault(config, "hostname", "default.com")
		assert.Equal(t, "default.com", val)
	})
}

func TestGetIntDefault(t *testing.T) {
	t.Parallel()

	t.Run("value exists", func(t *testing.T) {
		t.Parallel()
		config := Config{"port": 8080}
		val := GetIntDefault(config, "port", 443)
		assert.Equal(t, 8080, val)
	})

	t.Run("uses default", func(t *testing.T) {
		t.Parallel()
		config := Config{}
		val := GetIntDefault(config, "port", 443)
		assert.Equal(t, 443, val)
	})
}

func TestMustGetFloat(t *testing.T) {
	t.Parallel()

	t.Run("success with float64", func(t *testing.T) {
		t.Parallel()
		config := Config{"ratio": float64(3.14)}
		val, err := MustGetFloat(config, "ratio")
		require.NoError(t, err)
		assert.InDelta(t, 3.14, val, 0.001)
	})

	t.Run("success with int", func(t *testing.T) {
		t.Parallel()
		config := Config{"ratio": 5}
		val, err := MustGetFloat(config, "ratio")
		require.NoError(t, err)
		assert.Equal(t, float64(5), val)
	})

	t.Run("missing key", func(t *testing.T) {
		t.Parallel()
		config := Config{}
		_, err := MustGetFloat(config, "ratio")
		require.Error(t, err)
		assert.Contains(t, err.Error(), "ratio")
	})

	t.Run("wrong type", func(t *testing.T) {
		t.Parallel()
		config := Config{"ratio": "not a number"}
		_, err := MustGetFloat(config, "ratio")
		require.Error(t, err)
	})
}

func TestGetFloatDefault(t *testing.T) {
	t.Parallel()

	t.Run("value exists", func(t *testing.T) {
		t.Parallel()
		config := Config{"ratio": float64(2.5)}
		val := GetFloatDefault(config, "ratio", 1.0)
		assert.InDelta(t, 2.5, val, 0.001)
	})

	t.Run("uses default", func(t *testing.T) {
		t.Parallel()
		config := Config{}
		val := GetFloatDefault(config, "ratio", 1.0)
		assert.Equal(t, 1.0, val)
	})

	t.Run("wrong type uses default", func(t *testing.T) {
		t.Parallel()
		config := Config{"ratio": "not a float"}
		val := GetFloatDefault(config, "ratio", 1.0)
		assert.Equal(t, 1.0, val)
	})
}
